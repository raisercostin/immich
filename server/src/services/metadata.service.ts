...
export class MetadataService extends BaseService {
  ...
  private getExifTags(asset: { originalPath: string; files: AssetFile[]; type: AssetType }): Promise<ImmichTags> {
    if (asset.files.length > 0 && asset.type === AssetType.Image) {
  ...
  private async mergeExifTags(asset: {
    ...
    const [mediaTags, sidecarTags, videoTags] = await Promise.all([
      ...
      asset.files.length > 0 ? this.metadataRepository.readTags(asset.files[0].path) : null,
  ...
  private async processSidecar(id: string, isSync: boolean): Promise<JobStatus> {
    ...
    if (isSync && asset.files.length === 0) {
      return JobStatus.Failed;
    }

    if (!isSync && (asset.visibility === AssetVisibility.Hidden || asset.files.length > 0) && !asset.isExternal) {
      return JobStatus.Failed;
    }
